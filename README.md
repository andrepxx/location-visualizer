...
- `import-geodata host port name password format strategy input_file_path`: Import data from the geographical database to path `output_file_path`. Use format `format`, which may be any of `opengeodb`, `csv`, `json` or `gpx` or and import strategy `strategy`, which may be any of `all`, `newer` or `none`.
