...
function UI() {
	...
	this.displayActivities = function(div, response) {
		...
		const cgi = globals.cgi;
		downloadLinkHref.value = cgi + '?' + requestDownloadData;
		downloadLink.setAttributeNode(downloadLinkHref);
		const downloadLinkNode = document.createTextNode('Download CSV (*.csv)');
		downloadLink.appendChild(downloadLinkNode);
		downloadLinkDiv.appendChild(downloadLink);
		div.appendChild(downloadLinkDiv);
